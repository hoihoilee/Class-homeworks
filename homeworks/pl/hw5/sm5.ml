  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let rec find_locs (v1: value) =
        let rec check_base (l1, l_list: loc * (loc list)) =
          match l_list with
          | [] -> ()
          | h::t -> let (b1, _) = h in
                    let (b2, _) = l1 in
                    if ((b1 = b2) && ((List.mem h !reachable_locs) == false)) then
                    begin
                      reachable_locs := !reachable_locs @ [h];
                      find_locs (load h m)
                    end;
                    check_base (l1, t) in

        let rec mark_locs (l_list: loc list) =
          match l_list with
          | [] -> ()
          | h::t -> let (l2_list, _) = List.split m in
                    check_base (h, l2_list);
                    mark_locs t in

        match v1 with
        | L l1 -> mark_locs [l1]
        | R r1 -> let (_, l_list) = List.split r1 in
                  mark_locs l_list
        | _ -> () in
                            
      let rec mark_e (e_list: evalue list) =
        match e_list with
        | [] -> ()
        | h::t -> (match h with
                   | Loc l1 -> find_locs (L l1);
                               mark_e t
                   | Proc p1 -> let (_, _, env1) = p1 in
                                let (_, e_list) = List.split env1 in
                                mark_e e_list;
                                mark_e t) in

      let rec mark_s (s_list: svalue list) =
        match s_list with
        | [] -> ()
        | h::t -> (match h with
                   | V v1 -> find_locs v1;
      ...
      let rec mark_k (env_list: environment list) =
        match env_list with
        | [] -> ()
        | h::t -> let (_, e_list) = List.split h in
                  mark_e e_list;
                  mark_k t in
   
      let (_, e_list) = List.split e in
      let (_, env_list) = List.split k in
      mark_e e_list;
      mark_k env_list;
      mark_s s;
