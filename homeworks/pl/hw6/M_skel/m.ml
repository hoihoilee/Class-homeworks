...
struct
  ...
  let op2fn =
    ...
    | EQ -> (fun (v1, v2) -> match (v1, v2) with
                             ...
                             | (_, _) -> raise (TypeError "Invalid Comparison"))
  ...
  let rec eval env mem exp = 
    ...
    | LET (dec, e) -> (match dec with
                        | REC (f, x, e') -> eval (env @+ (f, (Closure (RecFun (f, x, e'), env)))) mem e
                        | VAL (x, e') -> let (v, m') = eval env mem e' in
                                         eval (env @+ (x, v)) m' e)
    | MALLOC e -> let (v, m') = eval env mem e in
                  let (l, m'') = malloc m' in
                  (Loc l, store m'' (l, v))
    | ASSIGN (e1, e2) -> let (v, m') = eval env mem e1 in
                         let (v', m'') = eval env m' e2 in
                         (v', store m'' (getLoc v, v'))
    | BANG e -> let (v, m') = eval env mem e in
                (load m' (getLoc v), m')
    | SEQ (e1, e2) -> let (v, m') = eval env mem e1 in
                      eval env m' e2
