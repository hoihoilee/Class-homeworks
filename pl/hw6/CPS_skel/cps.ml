...
let rec cps' exp = 
  ...
  | Ifz (e1, e2, e3) -> let v1 = new_name() in
                        let v2 = new_name() in
                        let v3 = new_name() in
                        Fn (k, App (cps' e1, 
                        Fn (v1, Ifz (Var v1, App (cps' e2,
                        Fn (v2, App (Var k, Var v2))), App (cps' e3,
  ...
  | Pair (e1, e2) -> let v1 = new_name() in
                     let v2 = new_name() in
                     Fn (k, App (cps' e1,
                     Fn (v1, App (cps' e2,
                     Fn (v2, App (Var k, Pair (Var v1, Var v2)))))))
  | Fst e -> let v1 = new_name() in
             Fn (k, App (cps' e,
             Fn (v1, App (Var k, Fst (Var v1)))))
  | Snd e -> let v1 = new_name() in
             Fn (k, App (cps' e,
             Fn (v1, App (Var k, Snd (Var v1)))))
